...
def get_edges(n, adj_mat):
    edges = []
    for i in range(n):
        for j in range(i + 1, n):
            if i != j and int(adj_mat[i][j]) == 1:
                edges.append([i, j])
    return edges
...
EDGES = get_edges(N, ADJ_MAT)


def _effective_kwargs(state, kwargs):
    if state is None:
        return kwargs or {}
    return state.get("kwargs") or {}


async def _cooperative_sleep(duration, minimum=0.001):
    await asyncio.sleep(max(float(duration), minimum))


async def alveoles_propagate(state=None, **kwargs):

    live_kwargs = _effective_kwargs(state, kwargs)
    max_intensity = live_kwargs.get("max_intensity", 1023)
    ...
    while True:
        live_kwargs = _effective_kwargs(state, kwargs)
        propagate.max_intensity = live_kwargs.get("max_intensity", 1023)
        duration_step = live_kwargs.get("duration_step", 0.01)
        colors = propagate.set_color()
        for i, pin in enumerate(PWMS):
            pin.duty(int(colors[i]))
        await _cooperative_sleep(duration_step)


async def alveoles_phasm(state=None, **kwargs):
...
async def alveoles_walker(state=None, **kwargs):

    live_kwargs = _effective_kwargs(state, kwargs)
    numbers = int(live_kwargs.get("numbers", 1))
    max_intensity = live_kwargs.get("max_intensity", 1023)

    starting_idx = [random.randint(0, N - 1) for _ in range(numbers)]
    ...
    last_max_intensity = max_intensity

    while True:
        live_kwargs = _effective_kwargs(state, kwargs)
        duration_step = live_kwargs.get("duration_step", 0.1)
        numbers = int(live_kwargs.get("numbers", 1))
        max_intensity = live_kwargs.get("max_intensity", 1023)

        if numbers != last_numbers or max_intensity != last_max_intensity:
            starting_idx = [random.randint(0, N - 1) for _ in range(numbers)]
            propagate = MultiWalker(
                ADJ_MAT,
                starting_idx=starting_idx,
...
async def alveoles_random_fill(state=None, **kwargs):
...
async def alveoles_flash(state=None, **kwargs):

    live_kwargs = _effective_kwargs(state, kwargs)
    duration = live_kwargs.get("duration", 1)
    max_intensity = live_kwargs.get("max_intensity", 1023)
    ...
    while True:
        live_kwargs = _effective_kwargs(state, kwargs)
        propagate.duration = live_kwargs.get("duration", 1)
        propagate.max_intensity = live_kwargs.get("max_intensity", 1023)
        pause_between = live_kwargs.get("pause_between", 0)
        colors = propagate.set_color()
        for i, pin in enumerate(PWMS):
            pin.duty(int(colors[i]))
        await _cooperative_sleep(propagate.duration / propagate.n_steps)
        if propagate.steps == 0:
            for i, pin in enumerate(PWMS):
                pin.duty(0)
            await _cooperative_sleep(pause_between)


async def program(program_name, state=None, **kwargs):
    if program_name == "alveoles_propagate":
        await alveoles_propagate(state=state, **kwargs)
    if program_name == "alveoles_walker":
        await alveoles_walker(state=state, **kwargs)
    if program_name == "alveoles_phasm":
        await alveoles_phasm(state=state, **kwargs)
    if program_name == "alveoles_random_fill":
        await alveoles_random_fill(state=state, **kwargs)
    if program_name == "alveoles_flash":
        await alveoles_flash(state=state, **kwargs)
